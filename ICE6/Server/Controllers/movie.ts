...
export function DisplayMovieList(req: Request, res: Response, next: NextFunction): void
{
    ...
    .then((data)=>
    {
        res.status(200).json({success: true, msg: "Movie List Retrived and Display", data: data, token: null})
    ...
    if (id.length != 24)
        ...
        else
        {
            ...
            {   
                if(data)
                    {
                        res.status(200).json ({success: true, msg: "One Movie Retrieve and Displayed", data: data, token: null})

                    }
                    else
                    {
                        res.status(400).json({success: false, msg:"Movie not found", data: null, token: null});
...
export function UpdateToMovie (req: Request, res:Response, next: NextFunction): void
    ...
    if (id.length != 24)
        ...
        Movie.updateOne({_id: id},movieToUpdate)
            .then(() =>
            {
                res.status(200).json({success: true, msg: "Movie Update ", data: movieToUpdate, token: null});
            })
            .catch((err) =>
            {
                console.error(err);
